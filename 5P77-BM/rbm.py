...
class RBM(nn.Module):
    ...
    def visible_to_hidden(self, v):
        ...
        """
        p = torch.sigmoid(F.linear(v, self.W, self.h))#1/ 1+e^-(v*W+b^h)
        return p.bernoulli()
    ...
    def hidden_to_visible(self, h):
        ...
        #1/ 1+e^-(h*W^T+b^v), note: W is transposed here, contrary to the literature, since we're multiplying the vector * matrix, not matrix * vector, as in the literature
    ...
    def free_energy(self, v):
        ...
        """
        v_term = torch.matmul(v, self.v.t())#a^T * x
        w_x_h = F.linear(v, self.W, self.h)#W^T_j * x + b_j
        h_term = torch.sum(F.softplus(w_x_h), dim=1)#sum(log(1+ exp(w_x_h)))
        return torch.mean(-h_term - v_term)#why take the mean????
